# US-27234: Print | White Mail Reminder | Send to Print
...
**Prerequisite: ** 
When Apply print toggle is enable in the contract settings [MNPS]  
Then should only the 'Send to Print' option is show in the White Mail Reminder grid and action column 
...
**Application removed from grid (Status shouldn't changed)**
 
When the letter is successfully stored in the BLOB location for the selected application 
And the application should be removed from the 'White mail reminder' grid screen 
...
**Note:** When an item is sent to the print partner, it should **not remain in the queue awaiting a response**. Once the item is sent, it should be **immediately removed from the queue**, and the **status should be updated accordingly**.If there is any **failure response from the print partner**, it must be **captured and logged in the system audit** for traceability. 
**** 
...
Event Type / Name: White mail reminder letter downloaded 
Event Description: "Reference number" white mail reminder letter downloaded** ** 
